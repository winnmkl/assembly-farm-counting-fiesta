...
; AddScore: Add/update a score entry.
...
DrawFooter:
        ...
        mov     word [gfx_strptr], str_footer
        mov     word [gfx_y], 192
...
; Displays masked input (asterisks). On commit goes to P1BANNER/P2BANNER.
; On ESC goes back to P1NAME/P2NAME.
; ============================================================================
DoPassEntry:
        push    ax
        push    bx
        ...
        ; Clear the password buffer for this player
        cmp     bl, 1
        jne     .clr_pw2
        mov     di, g_p1pass
        jmp     .clr_pw_do
...
.pw_lbl_set:
        ...
        ; Draw asterisks for each char entered
        ; Build mask string in pw_mask_buf: g_inlen '*' chars then NUL
        push    bx
        xor     bh, bh
        mov     bl, [g_inlen]
        mov     di, pw_mask_buf
...
.pw_no_fi:
        ...
        cmp     al, '~'
        ja      .pw_redraw
        ; Store character (raw, case-sensitive)
        cmp     byte [g_inlen], 8
        jae     .pw_redraw
        cmp     bl, 1
        jne     .pw_st_p2
...
.pw_back:
        ; Go back to name entry for this player
        cmp     bl, 1
        jne     .pw_bk_p2
        mov     byte [g_state], ST_P1NAME
        jmp     .pw_bk_done
.pw_bk_p2:
        mov     byte [g_state], ST_P2NAME
.pw_bk_done:
        pop     di
        pop     cx
        pop     bx
...
.v_tok_dr:
        ...
        ; Add to high scores - save BOTH players after every match
        ; Save P1
        push    si
        push    di
        mov     si, g_p1name
...
.skip_reason:
        ...
        ; Save score(s) on game over
        cmp     byte [g_mode], MODE_2P
        je      .go_save_2p
        ; 1P mode: save P1 if score > 0
...
DoHiScores:
        ...
        ; --- List entries ---
        ; Use g_has_entries flag (separate from clobbered tmp_w)
...
.no_row_bg:
        ...
        ; Highlight rank 1 with gold tint
        push    bx
        cmp     bx, 0
        jne     .not_rank1
        push    dx
...
.rk_set:
        ...
        ; Name (column at x=70, max 12 chars)
        cmp     bx, 0
        jne     .nm_normal
        mov     byte [gfx_color], C_TITLE_Y
        jmp     .nm_set
...
; High score table: 10 entries x 18 bytes
SCORES_FILE:    db "SCORES.DAT",0
g_scores:       times (10*18) db 0
